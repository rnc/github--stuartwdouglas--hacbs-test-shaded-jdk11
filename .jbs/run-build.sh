...
if [ -n "true" ]; then
  ...
  mvn -B -e -s "/var/workdir/software/settings/settings.xml" -t "/var/workdir/software/settings/toolchains.xml" org.codehaus.mojo:versions-maven-plugin:2.8.1:set -DnewVersion="1.9" | tee /var/workdir/workspace/logs/enforce-version.log
fi
