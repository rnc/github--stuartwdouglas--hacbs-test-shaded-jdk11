...
if [ -f /mnt/trusted-ca/ca-bundle.crt ]; then
    cat /mnt/trusted-ca/ca-bundle.crt
    keytool -import -alias jbs-cache-certificate -keystore "$FILE" -file /mnt/trusted-ca/ca-bundle.crt -storepass changeit -noprompt
fi
if [ -f /etc/pki/ca-trust/extracted/pem/tls-ca-bundle.pem ]; then
    keytool -import -trustcacerts -file /etc/pki/ca-trust/extracted/pem/tls-ca-bundle.pem -keystore $FILE -storepass changeit -noprompt -v
fi
...
if [ -n "true" ]; then
  ...
  mvn -V -B -e -s "/var/workdir/software/settings/settings.xml" -t "/var/workdir/software/settings/toolchains.xml" org.codehaus.mojo:versions-maven-plugin:2.8.1:set -DnewVersion="1.9" | tee /var/workdir/workspace/logs/enforce-version.log
fi
